...
function build (gyp, argv, callback) {
  ...
  function copyLib () {
    ...
    var buildDir = path.resolve(nodeDir, buildType)
      , archLibPath = path.resolve(nodeDir, arch, 'iojs.lib')
      , buildLibPath = path.resolve(buildDir, 'iojs.lib')

    mkdirp(buildDir, function (err, isNew) {
      if (err) return callback(err)
      log.verbose('"' + buildType + '" dir needed to be created?', isNew)
      var rs = fs.createReadStream(archLibPath)
        , ws = fs.createWriteStream(buildLibPath)
      log.verbose('copying "iojs.lib" for ' + arch, buildLibPath)
      rs.pipe(ws)
