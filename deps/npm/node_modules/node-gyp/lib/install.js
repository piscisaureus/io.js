...
function install (gyp, argv, callback) {
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      function downloadLib (done) {
        log.verbose('on Windows; need to download `iojs.lib`...')
        var dir32 = path.resolve(devDir, 'ia32')
          , dir64 = path.resolve(devDir, 'x64')
          , libPath32 = path.resolve(dir32, 'iojs.lib')
          , libPath64 = path.resolve(dir64, 'iojs.lib')
          , libUrl32 = distUrl + '/v' + version + '/win-x86/iojs.lib'
          , libUrl64 = distUrl + '/v' + version + '/win-x64/iojs.lib'

        log.verbose('32-bit iojs.lib dir', dir32)
        log.verbose('64-bit iojs.lib dir', dir64)
        log.verbose('`iojs.lib` 32-bit url', libUrl32)
        log.verbose('`iojs.lib` 64-bit url', libUrl64)

        var async = 2
        mkdir(dir32, function (err) {
          if (err) return done(err)
          log.verbose('streaming 32-bit iojs.lib to:', libPath32)

          var req = download(libUrl32)
          if (!req) return
          req.on('error', done)
          req.on('response', function (res) {
            if (res.statusCode !== 200) {
            ...
            getContentSha(res, function (_, checksum) {
              contentShasums['iojs.lib'] = checksum
              log.verbose('content checksum', 'iojs.lib', checksum)
            })
        ...
        mkdir(dir64, function (err) {
          if (err) return done(err)
          log.verbose('streaming 64-bit iojs.lib to:', libPath64)

          var req = download(libUrl64)
          if (!req) return
          req.on('error', done)
          req.on('response', function (res) {
            if (res.statusCode !== 200) {
            ...
            getContentSha(res, function (_, checksum) {
              contentShasums['x64/iojs.lib'] = checksum
              log.verbose('content checksum', 'x64/iojs.lib', checksum)
            })
